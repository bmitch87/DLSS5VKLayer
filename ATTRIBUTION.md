...
| [Blueforcer/ComfyUI-DLSS5-Enhancer](https://github.com/Blueforcer/ComfyUI-DLSS5-Enhancer) | MIT | Estimating optical flow at a fixed target WIDTH rather than at a fraction of the frame, so the cost is the same at 1080p and 4K, with the vectors rescaled by the actual ratio after even-rounding (`dlss5/motion.py:44-52`, `:88-94`). Also the `NVSDK_NGX_PerfQuality_Value` mapping, which is what showed that our `PerfQualityValue = 3` was commented "Balanced" when 3 is UltraPerformance (`dlss5/settings.py:28-35`). Also that skin structure does nothing without the automatic mask, because the mask is what tells the model where skin is -- stated as a dependency between two controls rather than left for a user to discover (`nodes/settings_node.py:92-110`); confirmed here by sweeping both, and the mask now gates the control in the interface. Also the preset range they ship (`dlss5/settings.py:8-13`), which prompted sweeping ours: on this model build no preset from 0 to 15 changed the picture, so the range was left alone and the finding recorded instead. Also that a scene-change threshold belongs to the user rather than to the build, with the per-frame score returned beside it so the threshold can be judged against something (`dlss5/settings.py:121`, `nodes/settings_node.py:131-138`, `dlss5/motion.py:27`) -- ours was a compile-once environment variable with no readout at all, and is now a header field with its score and cut count published. Their 0.24 of full scale, about 61 of 255, is the only independent number available for our own 55. |
| [lisitskyaa/ComfyUI-DLSS5-NR](https://github.com/lisitskyaa/ComfyUI-DLSS5-NR) | MIT | That the model's style is a three-value enum -- `default`/`natural`/`cinematic` mapped to 0/1/2 and nothing above (`nodes.py:141-145`, `:154`). Raised as a reason to expose 3-6, which prompted measuring them here: styles 2 through 6 produce byte-identical output on this model, so the style is clamped to their three rather than four inert entries being added to the interface. Also that a motion-vector scale MULTIPLIES the stored vector to give pixels (`native/dlss5nr_bridge.cpp:539-547`), which is what showed that our "UV 0..1" and "Normalised" modes were each sending the reciprocal of the conversion they name -- at 1920 wide, a scale of about a millionth rather than the frame's width. |
| [KleberMotta/OptiScaler-DLSS5-MFG-RTX40](https://github.com/KleberMotta/OptiScaler-DLSS5-MFG-RTX40) | GPL-3.0 | That the risk of a setting belongs next to the setting rather than in a document (`OptiScaler.ini:1632-1640`, `:1645`). Ours is an implicit Vulkan layer that loads into every Vulkan process started while it is enabled, and the words `anti-cheat`, `EAC` and `BattlEye` appeared nowhere in the tree; the interface now says so where the switch is. |
| [grim-susemi/OptiScaler-Susemi](https://github.com/grim-susemi/OptiScaler-Susemi) | GPL-3.0 | The SHAPE of an inspection hold: arm it and the NEXT successful frame is the one captured, rather than whichever frame happens to be on screen when the control is reached (commit `3fda6263`). Ours already held frames and already kept the control out of saved settings for the same stated reason; what it did not have was the one-shot arm, which is now `holdFrame = 2`. Also the practice of syncing a menu's default with the runtime's rather than letting the two tables drift (commit `8451bf8d`), which is now checked mechanically: the binder compares every control's bounds against what `ShmInitDefaults` puts in its field. Also the honest caveat that running the model above the frame's resolution has no further samples to find when the input is a frame that has already been presented (commit `3fda6263`), which the "Model resolution" tooltip had not said. |
